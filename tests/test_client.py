...
def test_parse_device_cdn_shape_with_snapshot_url() -> None:
    """Attach a capture URL to a single-channel camera."""
    client = UniviewCloudClient(
        session=None,
        username="user",
        password="pass",
    ...
    device = client._parse_device(
        ...
        "https://example.test/live/doorbell.m3u8",
        "https://example.test/capture/doorbell.jpg",
    )

    assert device.snapshot_url == "https://example.test/capture/doorbell.jpg"
...
def test_parse_channel_shape_with_snapshot_url() -> None:
    """Attach a capture URL to a channel camera."""
    client = UniviewCloudClient(
        session=None,
        username="user",
        password="pass",
    ...
    device = client._parse_channel(
        ...
        {"channelNo": "2", "channelName": "Kitchen", "status": 1},
        "https://example.test/live/channel-2.m3u8",
        "https://example.test/capture/channel-2.jpg",
    )
...
def test_snapshot_url() -> None:
    """Extract still image URLs from capture payloads."""
    assert (
        UniviewCloudClient._snapshot_url(
            {"url": "https://example.test/capture/channel-1.jpg"}
        )
        == "https://example.test/capture/channel-1.jpg"
    )
    assert (
        UniviewCloudClient._snapshot_url(
            {"imageUrl": "https://example.test/capture/channel-2.jpg"}
        )
        == "https://example.test/capture/channel-2.jpg"
    )
    assert UniviewCloudClient._snapshot_url({"expireTime": 123}) is None
